//******************************
 ...
 //*
 //*
 //*The RX pin on the sensor connects to pin D5 on the Wemnos D1 mini
 //*The TX pin on the sensor connects to pin D6 on the Wemnos D1 mini
 //
 //*Version：V3.1
 //*Author：Zuyang @ HUST
 //*Date：March.25.2016
...
void loop()
{
 ...
 digitalWrite(D3,HIGH); // to put it in constant operation

  
  if(PMSerial.find(0x42)){    
  
     PMSerial.readBytes(buf,LENG);
     // Serial.println(PMSerial.readBytes(buf,LENG));
     // Serial.println(buf[0]); 0x4d
    
 
    if(buf[0] == 0x4d){
     // if(checkValue(buf,LENG)){ // ne fonctionne pas
